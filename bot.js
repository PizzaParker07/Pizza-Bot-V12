...
client.on('roleDelete', async role => {
  ...
  require("discord-buttons")(client)
  {
    ...
    let data   = await db.get(`destekkullanıcı_${member.id}`);
    let data2  = await db.get(`destekkanal_${kanal.id}`);
    ...
    await db.set(`destekkullanıcı_${member.id}`, c.id);
    ...
    await db.delete(`destekkullanıcı_${button.clicker.user.id}`);
    ...
    await db.delete(`destekkullanıcı_${member.id}`);
    ...
    let data2   = await db.get(`destekkullanıcı_${data}`);
    //------------\\
    ...
    await db.delete(`destekkullanıcı_${data}`);
